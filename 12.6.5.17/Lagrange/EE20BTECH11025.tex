...
\begin{align}
	\max_{x}f\brak{x} &\triangleq V\brak{x} \label{eq:1} \\
...
\begin{align}
\implies \nabla L\brak{x,\lambda, \mu} &= 0 \label{eq:3}\\
...
Here we get
\begin{align}
\nabla f\brak{x} &=  (a-2x)(a-6x)\label{eq:4}\\
\nabla g_1\brak{x} &= -1\label{eq:5}\\
\nabla g_2\brak{x} &= 1\label{eq:6}
\end{align}
By using the equations \eqref{eq:4}, \eqref{eq:5}, \eqref{eq:6} we get
\begin{align}
