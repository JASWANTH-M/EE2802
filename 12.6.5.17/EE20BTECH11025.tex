...
\textbf{Solution:} Let the given side of tin be $a$.
\begin{align}
a = 18cm
\end{align}
...
\begin{align}
a-2x > 0,\,  x > 0\\
\implies 0 < x < \frac{a}{2} \label{eq:1}
\end{align} 
Volume of the box is given by
\begin{align}
V\brak{x} = x(a-2x)^2
\end{align}
We now check the convexity of the function $V\brak{x}$ under the constraints given by \eqref{eq:1}
\begin{align}
V^{\prime}\brak{x} &= (a-2x)(a-6x)\\
V^{\prime\prime}\brak{x} &= 8(3x - a)
\end{align}
For $x > \frac{a}{3}$ the function $V\brak{x}$ is convex, otherwise it is concave. The function convexity is changing under the constraints. 
So the given problem cannot be expressed as a Convex optimization problem. The problem can be solved using the Gradient Descent Algorithm. It can be modified as
\begin{align}
\min_{x}f(x) = -V\brak{x} \label{eq:2}
\end{align}
Here we have,
\begin{align}
f(x) &= -x(a-2x)^2 \label{eq:3}\\
f^{\prime}\brak{x} &= -(a-2x)(a-6x) \label{eq:4}
\end{align}
...
x = 3
\end{align}
From equation \eqref{eq:2}, the minimum value of $f\brak{x}$ is equivalent to maximum value of $V\brak{x}$. Hence the maximum value of $V\brak{x}$ occurs at $x = 3$.
\begin{align}
V\brak{3} = 432 
\end{align}
\end{enumerate}
